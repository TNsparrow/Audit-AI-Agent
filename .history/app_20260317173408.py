...
with st.sidebar:
    ...
    if uploaded_file:
        ...
        if uploaded_file.name not in st.session_state.processed_files:
            with st.spinner(f"发现新文件 {uploaded_file.name}，正在拼命阅读并合并到大脑中..."):
                # 保存临时文件
                ...
                st.success(f"《{uploaded_file.name}》已成功注入知识库！")
        
    # 在侧边栏直观地展示当前 AI 脑子里都装了哪些文件
    if st.session_state.processed_files:
        st.markdown("### 📚 当前已掌握的文档：")
